...
func GetSSOAccesstoken(w http.ResponseWriter, r *http.Request) {
	...
	ress3, err2 := db.Query("SELECT AppType, ifnull(Version,'')Version, tokenExpiredt, ifnull(name,'')name, ifnull(surename,'')surename, ifnull(department,'')department, ifnull(locationresponse,'')locationresponse, ifnull(permission,'')permission,ifnull(position,'')position,ifnull(keycloakToken,'')keycloakToken,ifnull(keycloakUserRole,'')Role FROM raotdb.vw_user WHERE username = '" + email + "' LIMIT 1 ")
